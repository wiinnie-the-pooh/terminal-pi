# Pi Coding Agent VS Code Extension — Testing Guide

This document describes how to verify `pi-agent` during development and before release.
...
2. Install it:
   ...
   ```sh
   code --install-extension pi-agent-*.vsix
   ```
...
| 5 | Set `piAgent.defaultArgs` to `"--thinking low"`; run Pi Coding Agent | Pi starts with `--thinking low` passed as arguments |
...
| 7 | In VS Code Stable, leave `piAgent.editorCommand` empty; run Pi and press `Ctrl+G` | Pi uses the Stable CLI default (`code --wait`) |
| 8 | In VS Code Insiders, leave `piAgent.editorCommand` empty; run Pi and press `Ctrl+G` | Pi uses the Insiders CLI default (`code-insiders --wait`) |
| 9 | In Cursor, leave `piAgent.editorCommand` empty; run Pi and press `Ctrl+G` | Pi uses the Cursor CLI default (`cursor --wait`) |
| 10 | In a known editor variant with its CLI missing from PATH, leave `piAgent.editorCommand` empty and pre-set `VISUAL` or `EDITOR` before launching VS Code | Pi Coding Agent preserves the inherited editor env instead of forcing a broken CLI command |
| 11 | Set `piAgent.editorCommand` to `code-insiders --wait`; run Pi and press `Ctrl+G` | Pi uses the configured editor command instead of the auto-detected default |
...
| 17 | Set `piAgent.virtualEnvironmentDrainMs` to `0`, open a Pi terminal | Venv activation text may reappear (expected: 0 disables the drain and races with ms-python) |
| 18 | Set `piAgent.virtualEnvironmentDrainMs` to `500`, open a Pi terminal | No venv activation; the restore of `python.terminal.activateEnvironment` happens ~500 ms after the terminal appears |
| 19 | Set `piAgent.virtualEnvironmentOverride` to `false`, open a Pi terminal in a Python workspace | The extension does not touch `python.terminal.activateEnvironment`; the workaround is disabled explicitly by the user |
...
| 24 | In Explorer, select a mixed set such as `SKILL.md` + `review.md` or `review.md` + `helper.ts` | The selection is ineligible as a whole; Pi Coding Agent must not partially process only the matching subset |
| 25 | In Explorer, include any folder in the selection | The selection is ineligible; Pi Coding Agent warns and does not launch |
...
| 29 | In an editor, invoke a mismatched command for the current file type (for example `Run Pi with Template...` on `SKILL.md`) | Pi Coding Agent warns and does not launch |
| 30 | Run `Pi Coding Agent: Run Pi with Skill...` from the Command Palette | Quick Pick lists only workspace `SKILL.md` files |
| 31 | Run `Pi Coding Agent: Run Pi with Template...` from the Command Palette | Quick Pick lists only workspace `.md` files excluding `SKILL.md` |
| 32 | Run `Pi Coding Agent: Run Pi with Extension...` from the Command Palette | Quick Pick lists only workspace `.ts` files |
...
| 35 | In Explorer, right-click a `.png` file and select "Run Pi with Prompt..." | Pi Coding Agent shows a warning and does not launch |
| 36 | In an editor with a `.ts` file open, run "Run Pi with Prompt..." from the editor right-click menu | Pi launches with `@<filepath>` |
| 37 | In an editor with a binary file open (e.g. `.png`), run "Run Pi with Prompt..." | Pi Coding Agent shows a warning and does not launch |
| 38 | Run `Pi Coding Agent: Run Pi with Prompt...` from the Command Palette | A native File Open dialog appears (not a Quick Pick) |
...
| 41 | Set `piAgent.promptExtraContext` to `focus on errors`; run "Run Pi with Prompt..." on any text file | Pi launches with `@<filepath> focus on errors` |
| 42 | Leave `piAgent.promptExtraContext` empty (default); run "Run Pi with Prompt..." | Pi launches with `@<filepath>` only -- no extra argument |
| 43 | With `piAgent.restoreSessionsOnStartup` enabled (default), open a workspace, run `piAgent.run`, say something to pi, then fully close VS Code | (setup step) |
...
| 47 | Set `piAgent.restoreSessionsOnStartup` to `false`; run Pi, close VS Code, reopen | No auto-restore; Pi does not open on startup |
| 48 | Run `piAgent.runWithSkill` with a skill file; close VS Code; reopen (restore enabled) | Pi terminal reopens and resumes the session via sidecar map; skill context is part of the resumed conversation |
...
If additional keys need to be forwarded to Pi, add a new entry to `package.json` under `contributes.keybindings` using `workbench.action.terminal.sendSequence`, the desired `key`, and the same `when` clause (`terminalFocus && piAgent.activeTerminal`). Add a corresponding one-line test in `tests/packageManifest.test.js` using `assertSendSequenceKeybinding(key, text)`. Finally, document the new key in the smoke-checklist table above.
