...
The extension always passes `--session <guid>` so each Pi terminal is tied to a specific session UUID. Pi stores the session in its default workspace directory (`~/.pi/agent/sessions/<encoded-cwd>/`). On restart, `--continue --session <guid>` resumes that session.
...
Each Pi terminal is launched with `--session <guid>` injected into its `shellArgs`, where `guid` is a `crypto.randomUUID()` value generated by the extension.

- Pi stores the session as `<timestamp>_<guid>.jsonl` in its default workspace directory (`~/.pi/agent/sessions/<encoded-cwd>/`), so the session is visible to `pi --resume` and other Pi tooling.
- When VS Code natively restores a terminal on restart, it re-runs the same shell command (including `--session <guid>`).
- `piLauncher.ts` searches `~/.pi/agent/sessions/` for a file matching `*_<guid>.jsonl`. If found, it prepends `--continue` so Pi resumes the existing session. If not found, all original args pass through for a fresh start.
